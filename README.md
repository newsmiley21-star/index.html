...
<head>
    ...
    <style>
        ...
        .search-box { background: white; border: 2px solid var(--gabon-bleu); padding: 10px; border-radius: 8px; margin-bottom: 15px; width: 100%; box-sizing: border-box; }

        table { width: 100%; border-collapse: collapse; font-size: 11px; }
        th { background: #444; color: white; padding: 8px; text-align: left; }
        td { padding: 8px; border-bottom: 1px solid #eee; }
        
        .card { border: 1px solid #ddd; padding: 12px; border-radius: 8px; margin-bottom: 10px; border-left: 6px solid var(--gabon-bleu); }
        ...
        .btn-del { color: var(--danger); border: none; background: none; font-weight: bold; font-size: 16px; padding: 5px; cursor: pointer; }
...
<body>
    ...
    <div id="main-app">
        <header>
            <h3 style="margin:0; color:var(--gabon-vert)">CT241 GABON</h3>
            <button id="btnOut" style="font-size:10px; color:red; background:none; border:none; font-weight:bold">DÉCONNEXION</button>
        </header>
        ...
        <div id="sec-saisie" class="section active-sec">
            ...
            <table>
                <thead><tr><th>ID</th><th>Client</th><th>Comm.</th></tr></thead>
        ...
        <div id="sec-reçus" class="section">
            <input type="text" id="searchInput" class="search-box" placeholder="🔍 Rechercher client, tél ou code TX..." oninput="filtrerRecus()">
...
<script type="module">
    ...
    window.lancerMission = () => {
        const n = document.getElementById('mNom').value, t = document.getElementById('mTel').value, r = document.getElementById('mRetrait').value, c = document.getElementById('mCom').value;
        if(!n || !r || !t) return alert("Nom, Téléphone et Montant requis");
        push(ref(db, 'missions'), {
            id: "OR-"+(idx+1).toString().padStart(3,'0'), nom: n, tel: t, lieu: document.getElementById('mLieu').value || "-", retrait: parseFloat(r), com: parseFloat(c) || 0,
            etape: 1, livreur: "Aucun", transaction: "", date: new Date().toLocaleDateString('fr-FR'), heure: new Date().toLocaleTimeString('fr-FR', {hour:'2-digit', minute:'2-digit'})
        });
    ...
    function ecouter() {
        onValue(ref(db, 'missions'), (s) => {
            ...
            afficherSaisieEtTaches();
            filtrerRecus();
        });
    }

    function afficherSaisieEtTaches() {
        const bS = document.getElementById('body-saisie'), lT = document.getElementById('list-taches');
        bS.innerHTML = ""; lT.innerHTML = ""; idx = 0;
        missionsLocales.slice().reverse().forEach(m => {
            ...
            if(m.etape === 1) {
                lT.innerHTML += `<div class="card">
                    ...
                    <input type="text" id="tx-${m.key}" placeholder="ID Transaction SMS...">
    ...
    window.filtrerRecus = () => {
        ...
        let tot = 0; lR.innerHTML = "";
        missionsLocales.slice().reverse().forEach(m => {
            if(m.etape > 1) {
                const searchTxt = (m.nom + m.tel + m.transaction + m.livreur).toLowerCase();
                    ...
                    lR.innerHTML += `<div class="card ${m.etape === 3 ? 'done' : ''}">
                        <div class="flex-card">
                            ...
                            <div style="display:flex; align-items:center; gap:10px;">
                                ${m.etape === 2 ? `<button onclick="encaisser('${m.key}')" style="background:var(--gabon-bleu); color:white; border:none; padding:8px; border-radius:5px">ENCAISSER</button>` : '✅'}
                                <button onclick="suppr('${m.key}')" class="btn-del">×</button>
                            </div>
                        </div>
                        <div class="tx-badge">TX: ${m.transaction || 'PAS DE CODE'}</div>
                        <div style="font-size:10px; margin-top:5px">Commission: ${m.com} FCFA</div>
                    </div>`;
                }
            }
        });
    ...
    window.valider = (k) => {
        ...
        if(!tx) return alert("Code SMS obligatoire !");
    ...
    window.suppr = (k) => { if(confirm("Supprimer définitivement ce reçu ?")) remove(ref(db, 'missions/'+k)); };

    window.shareWA = () => {
        let texte = "*BILAN CT241 GABON*\n\n";
        let total = 0;
