...
<body class="bg-gray-50 text-gray-900 font-sans">
    ...
    <script type="module">
        ...
        window.validerCommande = async () => {
            ...
            const mInput = document.getElementById('cMontant').value;
            const m = parseFloat(mInput);

            if(!n || !t || !l || isNaN(m)) return alert("⚠️ Veuillez remplir le formulaire.");
            ...
            // Constantes de calcul pour éviter le NaN dans l'app comptable
            const FRAIS_FIXES = 1190;
            const COM_LIVREUR = 800;
            const PROFIT_ADMIN = 390;
            ...
            try {
                ...
                const data = {
                    ...
                    lieu: l.trim(), 
                    retrait: Number(m),
                    frais: Number(FRAIS_FIXES),
                    total: Number(m + FRAIS_FIXES),
                    comLivreur: Number(COM_LIVREUR),
                    profitAdmin: Number(PROFIT_ADMIN),
                    benefice: Number(PROFIT_ADMIN), // Utilisé par la rubrique Reçu
                    commission: Number(FRAIS_FIXES), // Utilisé par la rubrique Reçu
                    etape: 1, 
                    livreur: "En attente", 
        ...
        window.rechercherCommande = () => {
            ...
            const myMissions = missionEntries.filter(([id, m]) => m.tel && m.tel.includes(tel));
            res.innerHTML = "";
            ...
            if(m.etape === 2) {
                ...
                if(m.gps) {
                    ...
                    initMap(m.gps);
                    document.getElementById('btnGoogleMap').onclick = () => {
                        window.open(`https://www.google.com/maps?q=${m.gps}`, '_blank');
                    };
                }
            } else if(m.etape === 3) {
                ...
                <div class="bg-white rounded-[2rem] p-6 shadow-xl border-2 border-gray-50 relative overflow-hidden">
                    ...
                    <div class="flex justify-between items-start mb-6">
                        <div>
                            ...
                            <h3 class="text-3xl font-black text-gray-800 tracking-tighter">${(Number(m.retrait) || 0).toLocaleString()} <span class="text-xs">F</span></h3>
                        </div>
        ...
        const livreurActiveId = urlParams.get('missionId');

        if(livreurActiveId && navigator.geolocation) {
            watchId = navigator.geolocation.watchPosition((pos) => {
                const lat = pos.coords.latitude;
                const lon = pos.coords.longitude;
                const newGps = `${lat},${lon}`;
                update(ref(db, `missions/${livreurActiveId}`), { gps: newGps });
